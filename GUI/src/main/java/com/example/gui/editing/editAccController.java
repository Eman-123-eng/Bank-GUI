...
import static BankManagement.BankAccount.writeToFile;
...
public class editAccController {
    ...
    public void initialize() {
        ...
        BankAccount ac = BankAccount.getAccount(id);
        accID_Field.setText(ac.getAcctID());
        No_Field.setText(ac.getAcctNo());
        typeField.setText(ac.getAcctType());
        balance_field.setText(String.valueOf(ac.getBalance()));
    }

    @FXML
    void editAcc(ActionEvent event) {
        BankAccount acc = BankAccount.getAccount(accID_Field.getText());
        acc.setAcctType(typeField.getText());
        acc.setBalance(Double.parseDouble(balance_field.getText()));
        writeToFile();
        acc_lbl.setText("Account is edited successfully!");
